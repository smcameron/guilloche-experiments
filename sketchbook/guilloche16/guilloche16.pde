...
float[] recx = { 10.0, 1000.0, 1000.0, 10.0, 10.0 };
float[] recy = { 900.0, 900.0, 10.0, 10.0, 900.0 };
float[] reclen = new float[5];
float totalreclen;
...
void setup() {  // setup() runs once
  ...
  totalreclen = calc_lengths(recx, recy, reclen, 5);
}
...
void draw()
{
  ...
  for (int i = 0; i < 1000; i++) {
    ...
    env4x = polygonx(t, 2 * PI / 10.0, recx, recy, reclen, 5, totalreclen) - ox;
    env4y = polygony(t, 2 * PI / 10.0, recx, recy, reclen, 5, totalreclen) - oy;
